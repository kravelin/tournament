...
def connect():
    ...
    try:
        ...
        raise IOError("Error connecting to the tournament database")
...
def playerStandings():
    ...
    query = "SELECT cg.ID, p.name, cg.wins, cg.matches FROM currentgame"
    query += " AS cg, players AS p WHERE cg.ID = p.ID ORDER BY cg.wins DESC"
    cursor.execute(query)
...
def swissPairings():
    ...
    for row in standings:
        if row[0] not in matched:
            ...
            pairing.append((row[0], row[1], competitor, standings[counter][1]))
...
def runTournament():
    ...
    while counter < rounds:
        ...
        for row in pairings:
            ...
            print("Match " + str(match) + " winner: " + row[winner + 1] +
                  " Loser: " + row[loser + 1])
            match += 1
        counter += 1
...
setupTournament()
while raw_input("\nWould you like to run another tournament? ") == "y":
    setupTournament()
